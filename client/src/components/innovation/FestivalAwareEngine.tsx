  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Festivals Tracked', value: FESTIVALS.length, icon: 'fa-calendar-days', color: 'bg-orange-50 text-orange-600' },
          { label: 'Est. Festival Spend', value: `₹${(totalFestivalSpend / 100000).toFixed(1)}L`, icon: 'fa-money-bill-wave', color: 'bg-rose-50 text-rose-600' },
          { label: 'Auto-Save Needed', value: `₹${monthlyAutoSave.toLocaleString()}/mo`, icon: 'fa-piggy-bank', color: 'bg-green-50 text-green-600' },
          { label: 'Days to Diwali', value: '145', icon: 'fa-fire', color: 'bg-amber-50 text-amber-600' },
        ].map((stat, idx) => (
          <motion.div
            key={stat.label}
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
              <i className="fas fa-om text-orange-600" /> Festival-Aware Financial Planner
            </h3>
            <p className="text-[11px] text-gray-500 mt-0.5">
              India's only banking engine that plans your finances around festivals — because in Bharat, festivals drive 40% of annual spending
            </p>
          </div>
          <button
          ...
          >
            <i className={`fas ${showPlanner ? 'fa-check' : 'fa-wand-magic-sparkles'} mr-1`} />
            {showPlanner ? 'Planner Active' : 'Auto-Plan All'}
          </button>
        </div>
                  ...
                  <div className="p-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <i className={`fas ${festival.icon}`} style={{ color: festival.color, fontSize: '15px' }} />
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-[12px] font-bold text-gray-800">{festival.name}</span>
                            <span className="text-[9px] px-1.5 py-0.5 bg-gray-100 rounded-full text-gray-500 capitalize">{festival.category}</span>
                          </div>
                          <div className="flex items-center gap-3 text-[10px] text-gray-500 mt-0.5">
                            <span><i className="fas fa-calendar mr-1" />{festival.date}</span>
                            <span><i className="fas fa-hourglass-half mr-1" />{festival.daysAway} days</span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mt-2.5">
                      <div className="flex items-center justify-between text-[9px] text-gray-400 mb-1">
                        <span>Countdown</span>
                        <span>{Math.round((1 - festival.daysAway / 365) * 100)}% through year</span>
                      </div>
                        ...
                        <div className="p-3 space-y-3">
                          <div className="flex items-start gap-2">
                            <div className="w-8 h-8 bg-orange-50 rounded-lg flex items-center justify-center flex-shrink-0">
                              <i className="fas fa-wand-magic-sparkles text-orange-500 text-xs" />
                            </div>
                            <div>
                              <p className="text-[10px] font-bold text-gray-700 mb-0.5">AI Recommendation</p>
                              <p className="text-[11px] text-gray-600 leading-relaxed">{festival.aiRecommendation}</p>
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="p-2 bg-gray-50 rounded-lg">
                              <p className="text-[9px] text-gray-400">Auto-Save/Week</p>
                              <p className="text-[11px] font-semibold text-gray-700">₹{festival.autoSaveAmount.toLocaleString()}</p>
                            </div>
                            <div className="p-2 bg-gray-50 rounded-lg">
                              <p className="text-[9px] text-gray-400">Fund Ready By</p>
                          ...
                          {showPlanner && (
                            ...
                            >
                              <i className="fas fa-robot mr-1" /> Auto-Setup Festival Fund
                            </motion.button>
                          )}
                        </div>
                      </motion.div>
      ...
      <div className="card-psb">
        <h4 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2">
          <i className="fas fa-chart-column text-primary" /> Festival Spending Impact on Annual Budget
        </h4>
        ...
        <div className="mt-2 flex items-center gap-4 text-[10px] text-gray-500">
          <span className="flex items-center gap-1"><div className="w-3 h-3 bg-primary/20 rounded" /> Normal</span>
          <span className="flex items-center gap-1"><div className="w-3 h-3 bg-amber-300 rounded" /> Festival Extra</span>
          <span className="flex items-center gap-1"><div className="w-3 h-3 bg-rose-400 rounded" /> High Spend Alert</span>
        </div>
      </div>
    </div>
  );
