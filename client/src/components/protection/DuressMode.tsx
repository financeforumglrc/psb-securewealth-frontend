...
export default function DuressModeToggle() {
  ...
  const toggleDuressMode = useWealthStore((s) => s.toggleDuressMode);
    ...
    <>
      ...
      <div className="card">
        ...
        <button
          ...
          className={`w-full py-2 rounded-lg text-sm font-medium transition-colors ${duressModeActive ? 'bg-rose-500 text-white hover:bg-rose-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200'}`}
        >
          {duressModeActive ? 'Deactivate Duress Mode' : 'Activate Duress Mode'}
        </button>
