  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Qubits', value: '16', icon: 'fa-atom', color: 'bg-blue-50 text-blue-600' },
          { label: 'Entropy', value: '99.9%', icon: 'fa-shuffle', color: 'bg-violet-50 text-violet-600' },
          { label: 'Encryption', value: 'PQ-Lattice', icon: 'fa-lock', color: 'bg-green-50 text-green-600' },
          { label: 'Threat Level', value: '0', icon: 'fa-shield-halved', color: 'bg-rose-50 text-rose-600' },
        ].map((stat, idx) => (
          <motion.div
            key={stat.label}
      ...
      <div className="card-psb">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
              <i className="fas fa-atom text-blue-600" /> Quantum Financial Vault
            </h3>
            <p className="text-[11px] text-gray-500 mt-0.5">
              Post-quantum lattice encryption protecting your wealth against future quantum computer attacks
            </p>
          </div>
        </div>
        ...
        <div className="p-4 bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl mb-4">
          <div className="grid grid-cols-4 gap-3 mb-3">
            {qubits.map((qubit) => (
              <motion.div
                ...
                transition={{ duration: 2, repeat: qubit.entangled ? Infinity : 0 }}
          ...
          <div className="flex items-center justify-between text-[10px] text-gray-400">
            <span><i className="fas fa-circle text-violet-400 mr-1 text-[6px]" /> Superposition</span>
            <span><i className="fas fa-circle text-blue-400 mr-1 text-[6px]" /> State 0</span>
            <span><i className="fas fa-circle text-rose-400 mr-1 text-[6px]" /> State 1</span>
            <span><i className="fas fa-circle text-green-400 mr-1 text-[6px]" /> Entangled</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          >
            <i className={`fas fa-${lockStatus === 'locked' ? 'check-circle' : lockStatus === 'locking' ? 'spinner fa-spin' : 'lock'} mr-2`} />
            {lockStatus === 'locked' ? 'Vault Secured' : lockStatus === 'locking' ? 'Encrypting...' : 'Activate Quantum Lock'}
          </button>
          {lockStatus === 'locked' && (
            <button
              onClick={resetLock}
        ...
        {lockStatus !== 'unlocked' && (
          <div className="mb-4">
            ...
            <p className="text-[10px] text-gray-400 mt-1 text-right">{Math.round(progress)}% encrypted</p>
          </div>
        )}
          ...
          {ENCRYPTION_STEPS.map((step, idx) => (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                ...
                <i className={`fas ${step.status === 'complete' ? 'fa-check' : step.status === 'active' ? 'fa-spinner fa-spin' : step.icon}`} />
              </div>
