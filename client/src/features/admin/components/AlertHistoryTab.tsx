...
import { can, type AdminRole } from '@/features/admin/lib/permissions';
...
export default function AlertHistoryTab({ role = 'superadmin' }: { role?: AdminRole }) {
                  ...
                  {selected.status === 'open' ? (
                    role === 'viewer' ? (
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 text-center">
                        <p className="text-xs font-semibold text-slate-600">View-only access</p>
                        <p className="text-[10px] text-slate-400 mt-0.5">Your role cannot modify alerts.</p>
                      </div>
                    ) : (
