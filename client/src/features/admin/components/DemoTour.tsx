...
export default function DemoTour({ onNavigate }: DemoTourProps) {
  ...
  if (!isOpen) {
    return (
      <motion.button
        ...
        animate={{ scale: 1 }}
        onClick={start}
        className={`fixed bottom-4 right-4 md:bottom-6 md:right-6 z-[9998] flex items-center gap-2 px-3 py-2 md:px-5 md:py-3 rounded-xl md:rounded-2xl shadow-lg hover:scale-105 transition-all border ${
  ...
  return (
    <>
      ...
      >
        <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden max-h-[80vh] md:max-h-[90vh] flex flex-col">
          {/* Progress bar */}
