...
const RISK_FACTORS_POOL = [
    ...
    'impossible_travel',
    'login_from_tor',
    'weekend_activity_spike',
    'rapid_multiple_hops',
    'shell_company_involved',
    'crypto_exchange_link'
];
...
function computeRiskScore(category, factors, pathHasSanctioned) {
    let base = { account_takeover: 78, mule_transfer: 85, card_fraud: 72, phishing: 68, insider: 60, identity_theft: 75, velocity: 70 }[category] || 65;
    base += factors.length * 3;
    if (pathHasSanctioned) base += 10;
    return Math.min(99, Math.max(30, base + randInt(-8, 8)));
}

function generateCase(index, total) {
    const idNum = index + 1;
    const now = new Date();
    ...
    const hops = [
        {
            hopNumber: 1,
            hopType: 'origin',
            nodeName: `${originCity.city}, India`,
            country: 'India',
            city: originCity.city,
            lat: originCity.lat + (Math.random() - 0.5) * 0.05,
            lon: originCity.lon + (Math.random() - 0.5) * 0.05,
            entityType: 'bank_account',
            entityValue: maskAccount(randInt(100000000000, 999999999999)),
            institution: sourceBank.name,
            ifsc: generateIfsc(sourceBank.code),
            amount: amountBase,
            currency: 'INR',
        ...
        },
        {
            ...
            nodeName: `${firstHop.city}, ${firstHop.country}`,
            country: firstHop.country,
            city: firstHop.city,
            lat: firstHop.lat + (Math.random() - 0.5) * 0.05,
            lon: firstHop.lon + (Math.random() - 0.5) * 0.05,
            entityType: 'bank_account',
            entityValue: maskAccount(randInt(1000000000, 9999999999)),
            institution: bankForCountry(firstHop.country).name,
            swiftBic: bankForCountry(firstHop.country).swift,
            ...
            isSanctioned: firstHop.sanctioned,
            evidenceJson: { swiftRef: `SWIFT${randInt(100000, 999999)}`, clearingTime: 'T+0' }
        },
        {
            hopNumber: 3,
            hopType: 'intermediate',
            nodeName: `${intermediate.city}, ${intermediate.country}`,
            country: intermediate.country,
            city: intermediate.city,
            lat: intermediate.lat + (Math.random() - 0.5) * 0.05,
            lon: intermediate.lon + (Math.random() - 0.5) * 0.05,
    ...
    if (hasCrypto) {
        ...
        hops.push({
            hopNumber: 4,
            hopType: 'exchange',
            nodeName: `${exchange.name}, ${exchange.country}`,
            country: exchange.country,
            city: exchange.country,
            lat: 0,
            lon: 0,
            entityType: 'crypto_exchange',
            entityValue: `WALLET-${Math.random().toString(36).slice(2, 10).toUpperCase()}`,
            institution: exchange.name,
            amount: Math.round(amountBase * 0.92),
            currency: 'USDT',
            timestamp: new Date(createdAt.getTime() + randInt(90000000, 172800000)).toISOString(),
            confidence: randInt(50, 80),
            isSanctioned: false,
    ...
    hops.push({
        ...
        hopType: 'destination',
        nodeName: `${final.city}, ${final.country}`,
        country: final.country,
        city: final.city,
        lat: final.lat + (Math.random() - 0.5) * 0.05,
        lon: final.lon + (Math.random() - 0.5) * 0.05,
        entityType: 'bank_account',
        entityValue: maskAccount(randInt(1000000000, 9999999999)),
        institution: rand(FOREIGN_BANKS.filter(b => b.country === final.country)).name,
        swiftBic: rand(FOREIGN_BANKS.filter(b => b.country === final.country)).swift,
        ...
        isSanctioned: final.country === 'Russia',
    ...
    const accounts = [
        {
            ...
            branch: `${originCity.city} Main Branch`,
            maskedAccount: maskAccount(randInt(100000000000, 999999999999)),
            ifsc: generateIfsc(sourceBank.code),
            country: 'India',
        ...
        },
        {
            ...
            bankName: bankForCountry(firstHop.country).name,
            branch: `${firstHop.city} Branch`,
        ...
        },
        {
            ...
            holderName: generateName(),
            bankName: bankForCountry(final.country).name,
            branch: `${final.city} Branch`,
            maskedAccount: maskAccount(randInt(1000000000, 9999999999)),
            swiftBic: bankForCountry(final.country).swift,
            country: final.country,
...
function seed(count = 500) {
    console.log(`Seeding ${count} synthetic fraud cases...`);
    const clear = db.prepare('DELETE FROM fraud_cases');
    clear.run();

    const insert = db.transaction(() => {
        for (let i = 0; i < count; i++) {
            const c = generateCase(i, count);
            const caseResult = fraudDb.createCase({
                caseRef: c.caseRef,
                status: c.status,
                priority: c.priority,
                ...
                countryRiskTags: c.countryRiskTags,
                createdAt: c.createdAt
            });
            const caseId = caseResult.lastInsertRowid;

            c.hops.forEach(h => fraudDb.createHop({ ...h, fraudCaseId: caseId, timestamp: h.timestamp }));
            c.accounts.forEach(a => fraudDb.createAccount({ ...a, fraudCaseId: caseId }));

            // Add a few notes to some cases
            if (Math.random() < 0.3) {
                fraudDb.createNote({
                    fraudCaseId: caseId,
                    adminId: c.assignedAdminId || 'admin_1',
                    note: `Initial triage: ${c.riskFactors.slice(0, 3).join(', ')} flagged.`
                });
            }
        }
    });
